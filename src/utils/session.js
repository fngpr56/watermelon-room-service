...
export function createSession(user) {
  // Store a simple expiry time inside the signed session payload.
  return {
    ...user,
...
export function serializeSessionCookie(session, secret, secure = false) {
  // The cookie stores the payload plus an HMAC signature to detect tampering.
...
export function readSessionFromRequest(req, secret) {
  ...
  // Compare signatures safely so broken or forged cookies are rejected.
  ...
  // Expired sessions are treated the same as missing sessions.
